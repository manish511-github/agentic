...
async def fetch_and_store_content_node(state: AgentState) -> Dict[str, Any]:
    ...
    for i in range(0, len(items_to_process), BATCH_SIZE):
        ...
        for item in batch:
            if item["type"] == "story":
                text_content = f"Title: {item.get('title', '')} {item.get('text', '') or ''}"
        ...
        try:
            ...
            logger.info(f"Upserted {len(points)} points to Qdrant collection '{collection_name}' (batch {i//BATCH_SIZE + 1})")
        except Exception as e:
